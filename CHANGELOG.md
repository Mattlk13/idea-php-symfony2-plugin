...
* Migrate `TwigIconProvider` to Kotlin and optimize badge rendering logic with pre-built icons for all combinations. (Daniel Espendiller)
* Add fallback for extracting Symfony console commands from compiled container XML (Daniel Espendiller)
* Improve Twig path resolution and support windows and unix path pattern (Daniel Espendiller)
* Handle `DumbMode` by returning empty resolve results in `XmlReferenceContributor` (Daniel Espendiller)
* Backport paid block name completion and navigation for `renderBlock`/`renderBlockView` in templates using optimized index based traversing (Daniel Espendiller)
* Introduce `KernelProjectDirParser` for extracting `kernel.project_dir` and normalize Twig paths (Daniel Espendiller)
* Use `ReadAction` to ensure thread-safe access when fetching Symfony console commands in "Run Anything" provider. (Daniel Espendiller)
* Filter services with #[Exclude] attribute from discovery in `ContainerCollectionResolver`. (Daniel Espendiller)
* Fix visibility of action shortcuts when plugin is disabled (Daniel Espendiller)
* [#2610](https://github.com/Haehnchen/idea-php-symfony2-plugin/issues/2610) Add support for Symfony 4.4+ template structures with updated completion logic and new tests (Daniel Espendiller)
* Support resource-based service definitions and optimize service loading in `ContainerCollectionResolver` (Daniel Espendiller)
* Add Vite entry point indexing and line marker support (Daniel Espendiller)
* Add Vite support for Twig templates with completion and navigation (Daniel Espendiller)
* Introduce file-level caching for Doctrine mapping drivers (Daniel Espendiller)
* Index Doctrine table names in `DoctrineMetadataFileStubIndex` (Daniel Espendiller)
* Remove legacy web deployment integration (Daniel Espendiller)
* Leverage `FileBasedIndex` for Twig block resolution to improve performance in `HtmlTemplateGoToDeclarationHandler`, `TwigHtmlLineMarkerProvider`, and `TwigBlockUtil`. (Daniel Espendiller)
* Replace custom virtual file visitor with `FileTypeIndex` for template discovery in `TwigUtil` to simplify code and improve performance. (Daniel Espendiller)
* [paid] Doctrine WITH inspection: Make SQL dependency optional via reflection (Daniel Espendiller)
* Optimize `getExtendsTemplates` by switching to `FileBasedIndex` for improved performance (Daniel Espendiller)
* Use `ReadAction` to ensure thread-safe access when fetching Symfony console commands in "Run Anything" provider. (Daniel Espendiller)
